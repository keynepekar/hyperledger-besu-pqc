...
public class HybridTransactionEncoder {
  ...
  public static void encode(final Transaction transaction, final RLPOutput out) {
    ...
    out.writeBytes(transaction.getTo().map(address -> address.getBytes().copy()).orElse(Bytes.EMPTY));
    out.writeUInt256Scalar(transaction.getValue());
    out.writeBytes(transaction.getPayload());
    writeAccessList(out, transaction.getAccessList());

    // PQC Fields
    out.writeByte(transaction.getPqcAlgorithmId().orElseThrow());
