...
public final class MLDSA44SignatureAlgorithm implements SignatureAlgorithmPQC {
  ...
  public PQCPublicKey createPublicKey(final Bytes encoded) {
    if (encoded.size() != PUBLIC_KEY_LEN) {
      throw new IllegalArgumentException("Invalid %s public key length: " + encoded.size());
    }
  ...
  public PQCSignature createSignature(final Bytes encoded) {
    if (encoded.size() != SIGNATURE_LEN) {
      throw new IllegalArgumentException("Invalid ML-DSA-44 signature length: " + encoded.size());
    }
  ...
  public boolean verify(
    ...
    try {
      MLDSAPublicKeyParameters params =
          new MLDSAPublicKeyParameters(MLDSAParameters.ml_dsa_44, publicKey.getEncoded().toArray());
